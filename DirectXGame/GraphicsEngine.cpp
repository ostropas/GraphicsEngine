...
GraphicsEngine::GraphicsEngine()
{
	...
	catch (...) { throw std::exception("TextureManager not created succesfuly"); }

	try {
		m_mesh_manager = new MeshManager();
	}
	catch (...) { throw std::exception("MeshManager not created succesfuly"); }
...
{
	GraphicsEngine::get()->getRenderSystem()->setRasterizerState(material->getCullMode() == CULL_MODE_FRONT);
